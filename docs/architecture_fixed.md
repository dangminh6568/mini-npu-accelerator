...
> **Target Workload:** MobileNet Inference — **Pointwise Conv 1×1** (chiếm ~90% compute) và Standard Conv2D qua im2col  
...
| Peak throughput | 12.8 GOPS @ 100 MHz |
...
- **Target workload là Pointwise Conv 1×1 và Standard Conv2D** — *không phải* Depthwise Conv. Depthwise Conv (3×3 kernel, channel_multiplier=1) khi qua im2col tạo ra ma trận rất "loãng" với K=9, buộc hardware chạy 2 passes (K=8 + K=1) → hardware utilization < 20%. Pointwise Conv 1×1 với K=C_in lớn là workload lý tưởng cho systolic array 8×8.
...
| 2 | **Internal SRAMs** | SRAM_W / SRAM_X / SRAM_Y — **Asymmetric Dual-Port RAM**: Port A (AXI) 32-bit, Port B (FSM) 64-bit |
...
> **⚠️ SRAM Port Width Mismatch:**
> AXI4-Lite data bus là **32-bit**, nhưng Systolic Array 8×8 cần nạp/xả **8 phần tử INT8 = 64-bit mỗi chu kỳ**. Các SRAM phải được implement là **Asymmetric Dual-Port RAM**:
> - **Port A** (kết nối AXI Slave): data width = **32-bit** — CPU ghi/đọc 2 lần AXI transaction để điền 1 hàng 64-bit.
> - **Port B** (kết nối NPU FSM): data width = **64-bit** — FSM đọc 1 dòng đầy đủ trong 1 chu kỳ và đẩy thẳng vào 8 thanh ghi Skewing FIFOs.
>
> Trong Xilinx Vivado, dùng Primitive `RAMB36E1` với cấu hình `READ_WIDTH_A ≠ READ_WIDTH_B` hoặc IP Block Memory Generator với chế độ Asymmetric.
...
> **⚠️ Giới hạn phần cứng — Hardware Tile Size cố định là 8×8:**
> Accumulation Buffer chỉ có **64× INT32** (tương đương 1 tile 8×8). Do đó, phần cứng NPU chỉ xử lý tối đa **M_hw = 8, K_hw = 8, N_hw = 8** trong một lần START.
>
> Các thanh ghi `DIM_M`, `DIM_K`, `DIM_N` (max 255) hoạt động ở **cấp độ Software Tiling** — CPU Host chịu trách nhiệm chia layer thành nhiều tile 8×8 và gọi START nhiều lần, **không phải** hardware tự xử lý toàn bộ ma trận M×K×N lớn trong một pass.
>
> **Ví dụ:** Layer có M=16, K=8, N=8 → CPU chia thành 2 lần START: lần 1 nạp A[0:8,:], lần 2 nạp A[8:16,:], rồi ghép kết quả SRAM_Y lại trong memory.
...
| `0x0008` | `DIM_M` | R/W | `0x8` | Số hàng tile hiện tại (≤ 8). **Giá trị hợp lệ: 1–8.** CPU tự chia M_total thành các tile ≤8 và gọi START nhiều lần. |
| `0x000C` | `DIM_K` | R/W | `0x8` | Inner dimension của tile hiện tại (≤ 8). **Giá trị hợp lệ: 1–8.** K_total > 8 được xử lý qua K-tiling với ACCUM_EN. |
| `0x0010` | `DIM_N` | R/W | `0x8` | Số cột tile hiện tại (≤ 8). **Giá trị hợp lệ: 1–8.** Bằng với số Output Feature Maps của tile. |
...
4. (Nếu K-tiling: ghi ACCUM_EN=1 cho intermediate tiles; ghi ACCUM_EN=0 cho final tile trước khi trigger START lần cuối)
...
```
    ...
    │           ▼                        │
    │    ┌──────────────┐                │
    │    │ST_STREAM_ACT │ M cycles       │
    │    │(stream act   │────────────────┘ (SOFT_RST từ bất kỳ trạng thái)
    │    │ qua skewing) │                │
    │    └──────┬───────┘                │
    │           │                        │
    │           ▼                        │
...
| **Diagonal Wavefront Compute** | T+10 → T+10+M−1 | **M cycles**: mỗi cycle stream 1 hàng activation qua Skewing FIFOs. Array WS 8×8 đã chốt weight → 1 hàng A/cycle, K được CPU quản lý bên ngoài qua K-tiling |
| **Drain + Deskew + Quantize** | T+10+M → T+10+M+16 | **17 cycles**: xả pipeline, đồng bộ, ReLU, scale, clamp, ghi SRAM_Y |
| **Done** | T+10+M+17 | `DONE=1`, `irq` bật, CPU bắt đầu đọc SRAM_Y từ offset `0x3000` |
...
// Max tích lũy lý thuyết: K_max×127×127 = 512×16129 ≈ 8.26M << 2^30
// Dải an toàn: ±2^30 (~1.07 tỉ) đủ margin cho mọi cấu hình hợp lệ
assert property (@(posedge clk) disable iff (!rst_n)
    ($signed(acc_out) >= $signed(-32'd1073741824)) &&
    ($signed(acc_out) <=  $signed( 32'd1073741823)))
...
| **Peak Throughput** | **12.8 GOPS @ 100 MHz** | 64 MACs/cycle × 2 Ops/MAC × 100M cycles/s |
